...
def get_dxdt(args, params):
    ...
    if args.ode_degree == 1:
        ...
        def weighted_sum(x):
            return tf.matmul(params['W'], x) + tf.reshape(tf.reduce_sum(input_tensor=params['W'], axis=1), [args.n_x, 1]) * x
    else:
...
def heun_solver(x, t_mu, dT, n_T, _dXdt, n_activity_nodes=None):
    ...
    dxdt_mask = tf.pad(tensor=tf.ones((n_activity_nodes, 1)), paddings=[[0, n_x - n_activity_nodes], [0, 0]])
    for _ in range(n_T):
