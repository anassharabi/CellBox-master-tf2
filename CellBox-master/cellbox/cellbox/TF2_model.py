...
class CellBox(PertBio):
    ...
    def build(self):
        ...
        if self.args.pert_form == 'by u':
            ...
            self.train_y0 = tf.transpose(a=self.train_x)
            self.monitor_y0 = tf.transpose(a=self.monitor_x)
            self.eval_y0 = tf.transpose(a=self.eval_x)
    ...
    def forward(self, y0, mu):
        ...
        mean, sd = tf.nn.moments(x=ys, axes=0)
        yhat = tf.transpose(a=ys[-1])
...
class CoExp(PertBio):
    ...
    def forward(self, x_gold, training, pos=None, idx=None):
        ...
        if training:
            ...
            xhats_avg = tf.reduce_mean(input_tensor=xhats, axis=1)
            return xhats_avg
    ...
    def get_ops(self):
        ...
        self.loss_mse = tf.reduce_mean(input_tensor=tf.square((self.train_x - self.xhat)))
        self.loss_mse_training = tf.reduce_mean(input_tensor=tf.square((self.train_x - self.xhat_training)))
        self.loss = self.loss_mse_training + self.l1_lambda * tf.reduce_mean(input_tensor=tf.abs(self.params['Ws']))
